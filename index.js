...
if (conf.AUTOBIO === 'yes') {
    setInterval(() => {
      ...
      zk.updateProfileStatus(
        `${conf.OWNER_NAME} 👻𝐑𝐞𝐝𝐦𝐢-𝐗𝐌𝐃👻  ${date.toLocaleString('en-US', { timeZone: 'Africa/Nairobi' })} It's a ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Nairobi' })}.`
      );
    }, 10 * 1000);
  }
...
function createNotification(deletedMessage) {
  ...
  let notification = `*😈𝐑𝐞𝐝𝐦𝐢-𝐗𝐌𝐃 ANTIDELETE👿*\n\n`;
  notification += `*Time deleted🥀:* ${new Date().toLocaleString()}\n`;
  notification += `*Deleted by🌷:* @${deletedBy.split('@')[0]}\n\n*Powered by 𝐑𝐞𝐝𝐦𝐢-𝐗𝐌𝐃*\n\n`;
  return notification;
}
